...
def create_cutout(fits_path, ra_start, dec_start, ra_end, dec_end, asteroid_id, obs_utc, ra_rate, dec_rate, v_mag):
    try:
        with fits.open(fits_path) as hdul:
            ...
            ax.imshow(cutout.data, cmap='gray', vmin=vmin, vmax=vmax, origin='lower')
            
            # Get world coordinates for markers
            start_coord = cutout.wcs.pixel_to_world(*start_cutout)
            end_coord = cutout.wcs.pixel_to_world(*end_cutout)
            
            # Plot markers with proper coordinates
            ax.plot(start_coord.ra.deg, start_coord.dec.deg, 'o',
                    color='lime', markersize=12, label='Start Position',
                    transform=ax.get_transform('world'))
            ax.plot(end_coord.ra.deg, end_coord.dec.deg, 's',
                    color='red', markersize=12, label='End Position',
                    transform=ax.get_transform('world'))
            
            # Connecting line
            ax.plot([start_coord.ra.deg, end_coord.ra.deg],
                    [start_coord.dec.deg, end_coord.dec.deg],
            ...
            steps = int(np.ceil(distance / fwhm_pixels)) + 1
            for t in np.linspace(0, 1, steps):
                cx = start_cutout[0] + t * dx_px
                cy = start_cutout[1] + t * dy_px
                
                # Calculate rotated corners
                rotated_corners = []
                for corner in corners:
                    rot_corner = rotation @ corner
                    px = cx + rot_corner[0]
                    py = cy + rot_corner[1]
                    coord = cutout.wcs.pixel_to_world(px, py)
                    rotated_corners.append((coord.ra.deg, coord.dec.deg))
                
                # Add polygon
                poly = Polygon(rotated_corners, closed=True, edgecolor='magenta',
            ...
            text = (f"Asteroid: {asteroid_id}\n"
                    ...
                    f"RA Rate: {ra_rate:.2f}\"/min\n"
                    f"Dec Rate: {dec_rate:.2f}\"/min\n"
                    f"Vmag: {v_mag:.2f}\n"
            ...
            # Save output
            os.makedirs(os.path.join(CUTOUTS_DIR, asteroid_id), exist_ok=True)
            output_path = os.path.join(CUTOUTS_DIR, asteroid_id, 
                                      f"{os.path.basename(fits_path).replace('.fits.fz', '')}_cutout.png")
...
# ... [Keep process_asteroid_motion and main unchanged]
...
def process_asteroid_motion(txt_path, asteroid_id):
    try:
        ...
        # Query Horizons for motion rates
        try:
            t = Time(obs_utc, format='iso', scale='utc')
            obj = Horizons(id=asteroid_id, location=LOCATION, epochs=t.jd)
            eph = obj.ephemerides()
            ...
            data = eph[0]
            ra_rate = data['RA_rate'] / 60  # "/hr to "/min
            dec_rate = data['DEC_rate'] / 60
            
            # Calculate endpoint coordinates (30s exposure = 0.5 minutes)
            delta_ra = (ra_rate * 0.5) / 3600  # Convert arcsec to degrees
            delta_dec = (dec_rate * 0.5) / 3600
            
            ra_end = metadata['ra_start'] + delta_ra
            dec_end = metadata['dec_start'] + delta_dec
            
            # Prepare new metadata entries
            new_entries = [
                f"\n# Asteroid motion calculations",
                f"RA Rate (\"/min): {ra_rate:.2f}",
                f"Dec Rate (\"/min): {dec_rate:.2f}",
            ...
            print(f"Motion Rates: RA {ra_rate:.2f}\"/min, Dec {dec_rate:.2f}\"/min")
            print(f"End RA/Dec: {ra_end:.6f}, {dec_end:.6f}")
            print(f"Total displacement: {delta_ra*3600:.2f}\" RA, {delta_dec*3600:.2f}\" Dec\n")
...
# main() function remains unchanged from original
def main():
